...
rule all:
	input:
		expand(dir1.joinpath("01.genome_diffhm", "{spl_pair}", "{spl_pair}.genome.png"), 
				spl_pair=spl_minus),
		expand(dir1.joinpath("02.bychr_diffhm", "{spl_pair}", "{spl_pair}.chr1.png"),
				spl_pair=spl_minus),
...
rule diff_mustache:
	...
	shell:
		...
		"{params.coolpup} loop {input.loop_cool[1]} {output.control_specific} {output.cloop_chm} ;\n"	

		"{params.coolpup} loop {input.loop_cool[1]} {output.treat_specific} {output.tloop_chm} ;\n"

		"{params.coolpup} loop {input.loop_cool[1]} {output.share_loops} {output.sloop_chm} ;\n"

		"{params.coolpup} loop {input.loop_cool[0]} {output.control_specific} {output.cloop_thm} ;\n"

		"{params.coolpup} loop {input.loop_cool[0]} {output.treat_specific} {output.tloop_thm} ;\n"

		"{params.coolpup} loop {input.loop_cool[0]} {output.share_loops} {output.sloop_thm} ;\n"
...
rule diff_diamond_domain:
	...
	params:
		...
		tad_classify = "/work/frasergen/backup/3d/project/Interactome/240590_human_20240717/04.HiC_diff/tad_classify.py",
	...
	shell:
		...
		"{params.coolpup} domain {input.tad_basic_cool[1]} {output.stable_region} {output.stable_control} ;\n"

		"{params.coolpup} domain {input.tad_basic_cool[1]} {output.merge_region} {output.merge_control} ;\n"

		"{params.coolpup} domain {input.tad_basic_cool[1]} {output.split_region} {output.split_control} ;\n"

		"{params.coolpup} domain {input.tad_basic_cool[1]} {output.rearr_region} {output.rearr_control} ;\n"
		
		"{params.coolpup} domain {input.tad_basic_cool[0]} {output.stable_region} {output.stable_treat} ;\n"
		
		"{params.coolpup} domain {input.tad_basic_cool[0]} {output.merge_region} {output.merge_treat} ;\n"
		
		"{params.coolpup} domain {input.tad_basic_cool[0]} {output.split_region} {output.split_treat} ;\n"

		"{params.coolpup} domain {input.tad_basic_cool[0]} {output.rearr_region} {output.rearr_treat} ;\n"
...
rule diff_diamond_boundary:
	...
	shell:
		...
		"{params.coolpup} border {input.tad_basic_cool[1]} {output.control_boundaries} {output.cc_ata} ;\n"

		"{params.coolpup} border {input.tad_basic_cool[0]} {output.control_boundaries} {output.ct_ata} ;\n"

		"{params.coolpup} border {input.tad_basic_cool[1]} {output.treat_boundaries} {output.tc_ata} ;\n"

		"{params.coolpup} border {input.tad_basic_cool[0]} {output.treat_boundaries} {output.tt_ata} ;\n"

		"{params.coolpup} border {input.tad_basic_cool[1]} {output.share_boundaries} {output.sc_ata} ;\n"

		"{params.coolpup} border {input.tad_basic_cool[0]} {output.share_boundaries} {output.st_ata} ;\n"
...
rule switch_plot:
	...
	params:
		python3 = software.get("python3"),
		cptmt_plot = "/work/frasergen/3D/work/shaojie/script/HiC/hic_prepare/standard_analysis/compartment_plot.py",
...
rule diff_cptmt:
	...
	params:
		...
		cptmt_switch = "/work/frasergen/backup/3d/project/Interactome/240590_human_20240717/04.HiC_diff/cptmt_switch.py",
...
rule diff_matrix_bychr:
	input: 
		incools = lambda wildcards: [single_dir.joinpath("04.matrix", f"{spl}_{compartment_res}","bychr",f"{spl}.chr1.zScore.matrix.gz") 
						for spl in wildcards.spl_pair.split(".minus.")],
	output:
		genome_figure = dir1.joinpath("02.bychr_diffhm", "{spl_pair}", "{spl_pair}.chr1.png")
	params:
		python3 = software.get("python3"),
		name_pair = lambda wildcards: wildcards.spl_pair.split(".minus."),
		#diff_matrix = biorep_dir.joinpath("diff_matrix.py"),
		...
		csize=single_dir.joinpath("01.ref","genome.chrsize"),
	run:
		shell(f"{params.python3} {params.diff_matrix} --matrix chrom --wt_mat {Path(input.incools[1]).parent} --case_mat {Path(input.incools[0]).parent} --wt_name {params.name_pair[1]} --case_name {params.name_pair[0]} --outdir {params.outdir_chrom} --res {loop_res} -g {params.csize}")
