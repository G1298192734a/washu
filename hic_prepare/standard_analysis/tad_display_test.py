...
def tad_plot(cool,bw,bed,region,outpref):
	bigwig=pyBigWig.open(bw)
	cool,bw,bed,region = map(fanc.load,(cool,bw,bed,region))
	majors=np.array((5,10,25,50,100,250))*1e6
	minors=np.array((1,10,100,1000))*1e6
	for i in region:
		...
		mj=majors[bisect_right(majors,m_dist)]//5
		mn=minors[bisect_right(minors,mj)-1]

		tad_cool=kplot.TriangularMatrixPlot(cool,vmax=0.005,max_dist=m_dist//2,colormap=sns.blend_palette(["white", "red"], as_cmap=True),draw_x_axis=False,ylabel=i.chrom,aspect=0.3)
...
def main(inref, binsize, cool_matrix, is_bw, tad_bed, outdir, point_num=10):
	...
	bw=pyBigWig.open(is_bw)
	sites=list()
	for i in list(bw.chroms()):
		stats=bw.stats(i,type="std",nBins=bw.chroms(i)//(binsize*100))
		stats=[j for j in stats if j!=None]
		if not stats:continue
		max_index=stats.index(max(stats))
		sites.append([i,binsize*100*(max_index+1),binsize*100*(max_index+2)])
	with open(outfile,"w") as outbuff:
		for i in sites:
			print(*i,sep="\t",file=outbuff)
	#with open(outfile, "w") as outbuff:
...
#			print(fa.name, plot_start, plot_end, sep="\t", file=outbuff)
	...
	tad_plot(cool_matrix, is_bw, tad_bed, outfile, Path(outdir).joinpath(spl_res))
